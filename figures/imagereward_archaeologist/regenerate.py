"""Render the ImageReward grid for the archaeologist prompt (3 rows x 5 cols)."""
...
REPO = HERE.parents[1]
DATA = REPO / "data" / "imagereward_archaeologist"
...
COLS = [
    ...
    ("Guided (damped)", "gns100_damp0.15"),
]
ROWS = [0, 1, 3]
PROMPT = r"``Photorealistic close-up of an archaeologist brushing weathered paper talismans in a flooded bunker, flashlight haze, focused expression''"
...
fig.savefig(HERE / "imagereward_archaeologist.pdf")
fig.savefig(HERE / "imagereward_archaeologist.png")
print(f"saved -> {HERE}/imagereward_archaeologist.{{pdf,png}}")
