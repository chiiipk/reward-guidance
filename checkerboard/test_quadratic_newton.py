...
def test_p2(reward_type="gaussian_bump", n_newton=0):
    D = 2
    ...
    for _ in range(n_newton):
        ...
        # z is 1x2, res is 1x2. solve expects vector or matrix.
        # Jac is 2x2.
        # We solve Jac * delta = res^T
    ...
    print(f"--- Reward: {reward_type}, n_newton: {n_newton} ---")
    print(f"Error in mean: {torch.abs(mean_grid - mu_new[0]).max().item():.2e}")
    print(f"Error in cov:  {torch.abs(cov_grid - Sigma_2nd).max().item():.2e}")

for n_newton in [0, 1, 2, 5]:
    test_p2("quadratic", n_newton)
for n_newton in [0, 1, 2, 5]:
    test_p2("gaussian_bump", n_newton)
